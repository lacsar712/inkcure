...
func (s *Server) Routes() http.Handler {
	...
	mux.HandleFunc("POST /dwell/start", withTimeout(s.handleStartDwell))
	mux.HandleFunc("POST /dwell/complete", withTimeout(s.handleCompleteDwell))
	...
	mux.HandleFunc("GET /inkvat/level", withTimeout(s.handleInkvatLevel))
...
func (s *Server) handleWarmup(w http.ResponseWriter, r *http.Request) {
	...
	writeOK(w, map[string]interface{}{
		"ready":  ready,
		"detail": detail,
		"dwell":  s.app.DwellRemaining(),
		"warmup": s.app.UvbankWarmupRemaining(),
	})
}

func (s *Server) handleStartDwell(w http.ResponseWriter, r *http.Request) {
	holder := r.URL.Query().Get("holder")
	if holder == "" {
		holder = "hmi-operator"
	}
	if err := s.app.StartDwell(r.Context(), holder); err != nil {
...
func (s *Server) handleCompleteDwell(w http.ResponseWriter, r *http.Request) {
	holder := r.URL.Query().Get("holder")
	if holder == "" {
		holder = "hmi-operator"
	}
	if err := s.app.CompleteDwell(r.Context(), holder); err != nil {
		writeErr(w, http.StatusConflict, err)
		return
	}
	writeOK(w, s.app.Snapshot())
