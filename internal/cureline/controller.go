...
func (c *Controller) Tick(ctx context.Context, snap model.PlantSnapshot, firing bool) (model.CurelineReading, error) {
	select {
	case <-ctx.Done():
		return snap.Cureline, fmt.Errorf("%w", model.ErrContextDone)
	default:
	}
	pressure, err := c.pressure.Compute(snap, firing)
	if err != nil {
		return snap.Cureline, err
	}
