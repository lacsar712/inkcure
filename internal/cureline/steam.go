...
func (s *SteamPath) ComputeFlow(snap model.PlantSnapshot, firing bool) float64 {
	...
	loadFactor := snap.Cureline.OutputMW / math.Max(snap.Settings.TargetMW, 1)
	pressureFactor := snap.Cureline.SteamPressurePSI / math.Max(snap.Settings.TargetSteamPSI, 1)
...
func (s *SteamPath) ComputeMW(snap model.PlantSnapshot, firing bool) float64 {
	...
	enthalpy := s.enthalpyFromPressure(snap.Cureline.SteamPressurePSI)
...
func (s *SteamPath) EnthalpyBalance(snap model.PlantSnapshot) float64 {
	inEnergy := snap.Inkvat.FeedwaterTPH * 200
	outEnergy := snap.Inkvat.SteamFlowTPH * s.enthalpyFromPressure(snap.Cureline.SteamPressurePSI)
...
func (s *SteamPath) LoadPercent(snap model.PlantSnapshot) float64 {
	...
	return (snap.Cureline.OutputMW / snap.Settings.TargetMW) * 100
}
