...
func (s *PlantStore) UpdateInkvat(unitID string, inkvat model.InkvatReading) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	snap, ok := s.units[unitID]
	if !ok {
...
func (s *PlantStore) UpdateUvbank(unitID string, comb model.UvbankReading) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	snap, ok := s.units[unitID]
	if !ok {
...
func (s *PlantStore) UpdateCureline(unitID string, cureline model.CurelineReading) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	snap, ok := s.units[unitID]
	if !ok {
