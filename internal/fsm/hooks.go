...
// LampframeDrivePulse counts lampframe-side effects from FSM after hooks (acceptance).
var LampframeDrivePulse func()

func RegisterLampframeDriveHook(chain *HookChain) {
	chain.OnAfter(func(ctx context.Context, from, to model.PlantState, event PlantEvent) error {
		_ = ctx
		_ = from
		_ = to
		_ = event
		if LampframeDrivePulse != nil {
			LampframeDrivePulse()
		}
		return nil
	})
