...
func NewDwellWindow(clk ProcessClock) *DwellWindow {
	return &DwellWindow{clk: clk, window: NewWindow(model.DwellWindow)}
}
...
type InkvatSwellWindow struct {
	clk    ProcessClock
	window Window
}

func NewInkvatSwellWindow(clk ProcessClock) *InkvatSwellWindow {
	return &InkvatSwellWindow{clk: clk, window: NewWindow(model.InkvatSwellSettleWindow)}
}

func (d *InkvatSwellWindow) Settled(swellAt time.Time) bool {
	return d.window.Satisfied(d.clk, swellAt)
}

func (d *InkvatSwellWindow) RequireSettled(swellAt time.Time) error {
...
type UvbankWarmupWindow struct {
	clk    ProcessClock
	window Window
}

func NewUvbankWarmupWindow(clk ProcessClock) *UvbankWarmupWindow {
	return &UvbankWarmupWindow{clk: clk, window: NewWindow(model.UvbankWarmupWindow)}
}

func (c *UvbankWarmupWindow) Ready(ignitionAt time.Time) bool {
	return c.window.Satisfied(c.clk, ignitionAt)
}

func (c *UvbankWarmupWindow) Require(ignitionAt time.Time) error {
